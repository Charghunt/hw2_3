...
# first query to find the row in studios for Warner Bros.
warnerbros = Studio.find_by({ "name" => "Warner Bros." })

# Insert rows for Studios
...
# Adds data for Movies & relevant associations
...
# Adds data for Actors & relevant associations
...
# first query to find the row in movies for select titles .
...
# then query to find the row in actors for select actors .
christianbale = Actor.find_by({"name"=> "Christian Bale"})
michaelcane = Actor.find_by({"name"=> "Michael Cane" })
...
# Adds data for Roles & relevant associations in Batman Begins
...
new_role2["actor_id"] = michaelcane["id"]
...
# Adds data for Roles & relevant associations in The Dark Knight
new_role6 = Role.new
...
new_role9["actor_id"] = michaelcane["id"]
...
new_role14 = Role.new
new_role14["movie_id"] = thedarkknightrises["id"]
new_role14["actor_id"] = annehathaway["id"]
